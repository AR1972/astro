...
M072 03/11/91 MD  mscode.asm    Fix NMI windows in INT 25/26 handlers.  Bug 6321.

M073 03/15/91 MD  dostab.asm    Change case mapping for char 231.

M074 03/18/91 MD  dostab.asm    Change size of Exec data block in Win 3.0 
                                instance data table.  Bug 6298.

M075 03/20/91 MD  dostab.asm    Revert to DOS 4.0 file name mapping for
                                y-umlaut and e-acute.

=============================================================================
changes after ESP2
=============================================================================

M076 08/29/91 SMR makefile.oak	Control OBJ file copying by making it dependent
				on VERSION.INC. B#2556.

M077 08/29/91 DBO makefile      Just rename loc macro to sto, for consistency
                  makefile.oak  with makefiles in other directories.

M078 09/09/91 SMR makefile	Added ROM Exclude Api. And made ROM find first
		  mstable.asm	and find next pay attention to ROM exclude list.
		  romfind.asm

M079 09/24/91 SMR msinit.asm	Skip the block device driver if it returns
				0 units supported

I001 10/01/91 JAH buf.asm	IBM fix to preserve CY when in buffer in HMA
				and there was an error reading a FAT sector
				5.0 PTR 722211 Bug #1483.
