...
PEN_Mess         db 13,10,'EMM386 Privileged operation error #xx -',13,10,'Deactivate EMM386 and Continue (C) or reBoot (B) (C or B) ? $'
PEN_Len          equ 0 + ($ - PEN_Mess)
PEN_Num          db ' '
POE_Mess         db 13,10,'EMM386: Unrecoverable privileged operation error #xx - press ENTER to reboot$'
POE_Len          equ 0 + ($ - POE_Mess)
POE_Num          db ' '
EXCPE_Mess       db 13,10,'EMM386 Exception error #xx - press ENTER to reboot$'
EXCPE_Len        equ 0 + ($ - EXCPE_Mess)
EXCPE_Num        db ' '
DMASIZE_Mess     db 13,10,'EMM386 DMA buffer is too small.  Add D=nn  parameter and reboot.$'
...
DMAMODE_Num      db ' '
WinBackfillMess  db 'xxx',13,10,'EMM386: Unable to start Enhanced Mode Windows due to base memory back fill.',13,10,'$'
...
WINEMM_Mess      db 'MICROSOFT           EMM386 '
DriverVersion    db '4.45         '
_R1_CODE ends
