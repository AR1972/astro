...
M032 03/18/91 MD  lmstub.asm    Bug 6298.  Correct length of Exec data block
                                in Win 2.x instance table.

m033 08/15/91 JAH versiona.inc  Changed revision number for DOS 5.0 to
                                revision b.

M034 08/16/91 NSM lmstub.asm	Bug#1742. Define a variable which is used
				by msinit.asm (MSDOS.SYS) while patching
				int 20-28 to an IRET.

;-------------------------------------------------------------------------------
; Changes after ESP2
;-------------------------------------------------------------------------------

M035 09/09/91 SMR lmstub.asm	Added ROM exclude API.
		  syscall.inc

M036 09/11/91 SMR	MULT.INC	B#2669. Registered POWER's 2f channels
			POWER.INC	in mult.inc
			POWER.ASM
			PTIME.ASM
			LPC.ASM

M037 10/25/91 EEA  versiona.inc  Changed revision number for DOS 5.0 back to
                                 revision A.

M038 11/05/91 JAH  dosseg.inc    Added conditionals to remove ROMDOS conditionals
                                 segments
