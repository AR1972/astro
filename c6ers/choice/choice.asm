...
MultPromptErr:
	...
	mov		bx, offset MULTPROMPTERRMSG
	call	StrOut
	mov		bx, offset SYNTAXMSG
	call	StrOut
	mov		al, ERROREXIT
	jmp		ChoiceDone
...
IsHelpSwitch:						; /?
	...
	mov		bx, offset HELPMSG0
	call	StrOut
	mov		bx, offset SYNTAXMSG
	call	StrOut
	mov		bx, offset HELPMSG1
	call	StrOut
	mov		al, ERROREXIT
	jmp		ChoiceDone
...
ChoiceSwDone:						; Have all choices now.
	...
	jne		Parse1
	mov		bx, offset NEEDCHOICE	; No choices specified.
	call	StrOut
	mov		al, ERROREXIT
	jmp		ChoiceDone
...
;  ********************************************************************
	...
	repnz	scasb
	jz		TimeOutValid
	mov		bx, offset BADTIMEOUTCHARMSG	; char not in choices.
	call	StrOut
	mov		al, ERROREXIT
	jmp		ChoiceDone
...
GetKey1:
	...
	cmp		[CaseSense], TRUE		; Unless CaseSense, convert to ucase
	je		GetKey2
